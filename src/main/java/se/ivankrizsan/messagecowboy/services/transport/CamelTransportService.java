...
public class CamelTransportService extends AbstractXmlConfigurerdTransportService {
	...
	@Override
	public synchronized void dispatch(final MoverMessage inMessage, String inEndpointURI)
	...
	@SuppressWarnings("rawtypes")
	@Override
	public synchronized MoverMessage receive(String inEndpointURI, long inTimeout)
			throws TransportException {
	...
	public synchronized void refreshConnectors() throws IOException {
        ...
        if (theConfigRsrcChangedFlag) {
    		...
    		mCamelSpringContext = new FileSystemXmlApplicationContext(
    				mConfigResourcesLocationPatterns.toArray(
    						new String[mConfigResourcesLocationPatterns.size()]));
    		...
    		try {
    			...
    			BlockingCamelStarter theBlockingCamelStarter 
    				= new BlockingCamelStarter(mCamelContext);
    		...
    		LOGGER.info("Message Cowboy Transport using Apache Camel {} - Status: {}",
    				mCamelContext.getVersion(),mCamelContext.getStatus().toString());
	...
	class BlockingCamelStarter implements Future<Boolean>, StartupListener{
		 ...
		 */
		public BlockingCamelStarter(CamelContext inCamelContext){
